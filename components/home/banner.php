        ...
        const getData = async () => {
            try {
                ...
                if (data?.data?.length > 0) {
                    const dataBanner = data.data.find(element => element.type === 'bg_carousel')
                    if (dataBanner?.images?.length > 0) {
                        if (dataBanner?.images?.length === 1) {
                            document.getElementById('swiper-button').style.display = 'none'
                        }

                        dataBanner?.images?.sort((a, b) => a.file_name.localeCompare(b.file_name))?.forEach((image, i) => {
                            ...
                            bannerImage.innerHTML = `<img src="${image.url_path}" alt="${image?.image_description ?? getImageFileName(image.url_path) ?? ""}" class="w-full height-auto object-cover object-center"/>`
