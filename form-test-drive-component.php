<div class="w-full bg-white mb-20 px-5 pt-20 sm:mb-20 lg:pr-24 lg:pl-20 lg:pt-32 lg:mb-[22.5rem] xl:mb-40">
    <div class="w-full flex flex-wrap">
        <div class="w-full mb-4 sm:mb-0 md:w-5/12">
            <img src="images/form-test-drive-image.png" alt="test drive" class="w-full rounded-lg object-cover object-center h-auto" />
        </div>
...
<script>
    ...
    const getBranch = async () => {
        try {
            let response = await fetch('https://cms.bydhaka.co.id/api/v1/branch')
    ...
    const getProduct = async () => {
        try {
            let response = await fetch('https://cms.bydhaka.co.id/api/v1/product')
            const data = await response.json();
    ...
    function handleSubmit(event) {
        ...
        if (!fullName.value || !isValidPhone || !cars.value || !branch.value || formatDateYYYYMMDD(date.value).includes('NaN') || !time.value || !terms.checked) {
            ...
            const url = `https://cms.bydhaka.co.id/api/v1/test-drive`
            const formData = {
